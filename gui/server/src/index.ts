...
function _errorHandler(err: string): void {
    console.error(err);
}

function _messageHandler(data: Buffer): void {
    let parsed: { type: string, data: {}} = JSON.parse(data.toString());
    switch(parsed.type) {
        case "GET_DATA":
            sendData();
            break;
